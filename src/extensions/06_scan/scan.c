...
Datum full_table_scan(PG_FUNCTION_ARGS)
{
    ...
    tablescandesc = table_beginscan(relation, snapshot, 0, NULL);

    /* Log the attribute names of the table */
    for (int i = 0; i < tupdesc->natts; i++)
    {
        char *attname = NameStr(TupleDescAttr(RelationGetDescr(relation), i)->attname);
...
Datum table_scan_with_scankeys(PG_FUNCTION_ARGS)
{
    ...
    /* Perform a full table scan */
    snapshot = RegisterSnapshot(GetTransactionSnapshot());
    tablescandesc = table_beginscan(relation, snapshot, 2, scanKeys);
...
Datum table_scan_with_index(PG_FUNCTION_ARGS)
{
    ...
    snapshot = RegisterSnapshot(GetTransactionSnapshot());
    indexscandesc = index_beginscan(relation, indexrelation, snapshot, NULL, 2, 0);
